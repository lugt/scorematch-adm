...
public class SHA3Digest {
    ...
    public SHA3Digest(SHA3Digest source) {
        ...
        this.chunk = Arrays.copyOf(source.chunk, source.chunk.length);
        this.oneByte = Arrays.copyOf(source.oneByte, source.oneByte.length);
    }
    ...
    private void initSponge(int rate, int capacity) {
        ...
        this.rate = rate;
        // this is never read, need to check to see why we want to saveDuration it
...
//            displayIntermediateValues.displayStateAs64bitWords(3, "After iota", state);
    ...
    public static String StringDigest(int bits, String expected) throws Exception {
        SHA3Digest digest = new SHA3Digest(bits);
        byte[] hash = new byte[digest.getDigestSize()];
        byte[] k64 = expected.getBytes();
